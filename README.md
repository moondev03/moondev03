<br>

<picture>
  <source
    srcset="https://github.com/user-attachments/assets/3b9f38c5-70be-4d54-9eb4-0ae779440b50"
    media="(prefers-color-scheme: dark)"
  >
  <img
    src="https://github.com/user-attachments/assets/cae40dc6-5869-4c01-b580-b75387791fef"
    alt="profile-intro"
    style="width:60%; max-width:720px;"
  >
</picture>

<br><br>
      
### Education

    우아한테크코스 7기 (안드로이드 과정) | 2025.02 ~ 2025.12
    
    명지대학교 (데이터테크놀로지전공) | 2019.03 ~ 2025.02

<br>

### Activity

    명지대학교 IT 프로젝트 동아리 DEPth
        ㄴ 4기 운영진 · 안드로이드 파트장 | 2025.02 ~ ing
        ㄴ 3기 운영진 · 안드로이드 파트장 | 2024.02 ~ 2025.02
        ㄴ 2기 안드로이드 파트원 | 2023.09 ~ 2024.01
        
    수익화 IT 커뮤니티 Prography
        ㄴ 9기 안드로이드 파트원 | 2024.02 ~ 2024.09

<br>

### Project

#### [보따리 - 여행부터 일상까지, 내 손안의 체크리스트](https://github.com/woowacourse-teams/2025-bottari)
ㄴ `Android` · `2025.07 ~ ing` · `개인 및 단체를 위한 체크리스트 서비스`

#### [띵커벨 - 명지대학교 공지사항 알림 서비스](https://github.com/Team-Never-Land/ThinkerBell-Android) 
ㄴ `Android` · `2024.07 ~ ing` · `명지대학교 공지사항 통합 서비스`

<br>
<details>
<summary><strong>Archive</strong></summary>

---

#### [3days - 직장인 소개팅](https://github.com/Student-Center/3days-aos) 
ㄴ `Android` · `2024.08 ~ 2025.03` · `개발 중단` · `직장인 대상 소개팅 서비스`

#### [일거수일투족](https://github.com/Move-Log/Move-Log-Android) 
ㄴ `Android` · `2024.09 ~ 2025.02` · `서비스 종료` · `행동 기록 기반 AI 뉴스 이미지 제작 서비스` · `DEPth 3기 DemoDay 1위`

#### [RoomE - 나만의 방탈출 라이프](https://github.com/Prography-RoomE/roome_aos) 
ㄴ `Android` · `2024.03 ~ 2024.12` · `서비스 종료` · `방탈출 카페 취향 기반 프로필 생성 서비스`

#### [나의 비밀 친구, 나비](https://github.com/MJU-SW-Contest-2024/Nabi-Android) 
ㄴ `Android` · `2024.07 ~ 2024.08` · `일기 기반 힐링 챗봇 서비스` · `명지대학교 제 3회 창의적 SW 경진대회 총장상(대상)`

#### [WEAVE - 안전한 대학생 미팅 플랫폼](https://github.com/Student-Center/weave-aos) 
ㄴ `Android` · `2024.01 ~ 2024.05` · `서비스 종료` · `대학생 대상 미팅 플랫폼`

#### [전시로그 - 전시를 기록하다](https://github.com/AndLetgo/2nd-Main-JeonsiLog-Client) 
ㄴ `Android` · `2023.09 ~ 2024.08` · `서비스 종료` · `전시 정보 / 리마인더 / 감상 기록 플랫폼` · `DEPth 2기 DemoDay 1위`

---

</details>
